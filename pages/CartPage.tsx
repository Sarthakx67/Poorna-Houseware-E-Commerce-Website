...
const CartPage: React.FC = () => {
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl mx-auto">
      <h1 className="text-4xl font-extrabold text-brand-primary mb-8 text-center">Your Order</h1>
      {itemCount === 0 ? (
        <div className="text-center py-12">
          <p className="text-brand-secondary text-lg mb-6">Your order is empty.</p>
          <Link to="/" className="bg-brand-primary hover:bg-brand-primary-hover text-white font-bold py-3 px-6 rounded-lg transition-colors shadow-lg hover:shadow-xl">
            Continue Browsing
          </Link>
        </div>
      ) : (
        <>
            ...
            {cartItems.map(item => (
              <div key={item.id} className="py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex-grow">
                  <p className="font-bold text-brand-secondary">{item.productName}</p>
          ...
          <div className="mt-8 border-t pt-8">
            <label htmlFor="siteLocation" className="block text-lg font-semibold text-brand-secondary mb-2">
          ...
          <div className="mt-8 text-center">
             <button 
                ...
                className="w-full max-w-md bg-green-500 hover:bg-green-600 text-white font-bold py-4 px-6 rounded-lg transition-all duration-300 flex items-center justify-center text-xl shadow-lg hover:shadow-xl transform hover:-translate-y-1"
              >
